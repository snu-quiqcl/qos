...
#[repr(C)]
pub struct SCURegs {
    // Snoop Control Unit: MPCORE_VIRT0
...
#[repr(C)]
pub struct GICCRegs {
    // GIC CPU Interface: MPCORE_VIRT0 + 0x0000_0100
...
pub struct TimerRegs {
    // Timers: MPCORE_VIRT0 + 0x0000_0200
...
#[repr(C)]
pub struct GICPRegs {
    // Peripherals: MPCORE_VIRT1 + 0x0000_0d00
...
impl Mpcore {
    ...
    pub fn irq_end_interrupt(&mut self, irqid: u32) {
        ...
        eoi += irqid;
        unsafe {
            self.cpu_interface.eoir.write(eoi);
        }
    }
    ...
    pub fn ptc_start_timer(&mut self, mode: u32, prescaler: u8, load: u32) {
        unsafe {
            ...
            self.timer
                .ptccr
                .write((prescaler as u32) << 8 | 1 << 2 | mode);
        }
    }
